...
struct AnnotateArgs {
    ...
    /// Only annotate the first occurence of a word.
    #[arg(long, short = '1')]
    only_first: bool,
}
...
fn output(output_path: Option<AsRef<Path>>) -> Box<dyn Write> {
...
async fn main() {
    ...
    match args.command {
        Command::Annotate(a) => {
            ...
            let formatter = {
                if a.katakana {
                    Box::new(WithKanakana(&*formatter))
                } else {
                    formatter
                }
            };
            ...
            let selector = match (a.only_first, a.include_common) {
                (true, true) => {
                    Box::new(select::filter::FirstOccurence::new(select::heuristic::All))
                        as Box<dyn Select>
                }
                (true, false) => Box::new(select::fileter::FirstOccurence::new(
